...
├── hooks/
│   ├── session-context.sh             # 프로젝트 스냅샷 생성기 + 신선도 검사
│   └── context-fingerprint.sh         # 구조적 파일 내용 지문 생성기 (머클 트리 역할)
└── skills/
...
2. **지문 불일치 감지 (실시간)** — 문서 생성 시점에 구조적 파일(매니페스트·
   CI·빌드 설정)의 내용 해시와 디렉터리 구조 해시를 지문으로 저장하고, 매 세션
   훅이 **작업 트리의 현재 내용**과 비교합니다. 커서의 머클 트리 비교와 같은
   원리라서 커밋 수와 무관합니다: 미커밋 수정도 잡히고, 리베이스·브랜치 전환도
   잡히고, 롤백으로 문서 시점 상태에 돌아오면 지문이 다시 일치해 불필요한
   갱신이 일어나지 않습니다
