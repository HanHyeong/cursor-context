...
이 프로젝트의 `.claude/settings.json`에는 스킬이 게이트 스크립트를 승인
프롬프트 없이 돌릴 수 있게 `permissions.allow`에 `Bash(.claude/hooks/*)`
규칙이 등록돼 있는데, 이 규칙은 Bash 도구에 넘어가는 **명령 텍스트의
문자 그대로**를 접두어 매칭한다(셸 변수 확장을 전혀 이해하지 못하며,
Claude Code 공식 문서가 명시하는 동작이다). 명령을 변수로 바꾸면 텍스트가
더 이상 `.claude/hooks/`로 시작하지 않아 매칭이 깨지고, 매 호출마다
권한 프롬프트가 뜬다 — 프롬프트 없는 세션에서는 그 순간 진화가 조용히
중단된다.

install.sh 배치(대부분의 경우)에서는 아래 명령을 그대로 실행한다. **그
경로에 스크립트가 없다면**(플러그인 배치) 이 SKILL.md의 실제 위치 기준
`../../hooks/<스크립트>`로 경로만 바꿔 그대로(변수 없이) 실행하라 — 이
경우 위 허용 규칙이 적용되지 않아 권한 프롬프트가 뜰 수 있다(플러그인
배치의 알려진 한계이며 정상 동작이다). 두 배치 모두 `skills/<이름>/`과
`hooks/`가 같은 루트의 형제 디렉터리라 이 상대 경로는 항상 성립한다.
