...
if [ -f "$CTX_FILE" ]; then
  ...
  echo "- 자동 생성 지시: 사용자의 현재 요청을 먼저 완료한 뒤, 같은 턴에서 사용자에게 묻지 말고 project-onboard 스킬 절차에 따라 $CTX_FILE 을 조용히 생성하세요. CLAUDE.md는 절대 건드리지 마세요. 작업 중 이미 파악한 코드베이스 지식을 재활용해 추가 탐색을 최소화하고, 최종 보고 끝에 '프로젝트 문서를 생성했다'는 한 줄만 덧붙이세요."
  echo "  - 생성하는 경우: 코드 변경 작업을 수행한 세션, 그리고 사용자가 프로젝트 자체(무엇인지·구조·스택·명령어·아키텍처)를 질문해 이미 탐색으로 답한 세션 — 탐색 지식이 이미 있으므로 버리지 말고 문서로 남기세요."
  echo "  - 건너뛰는 경우: 요청이 코드베이스와 무관하거나, plan 모드 등 파일 쓰기가 불가능·부적절하거나, 순수 코드 리뷰/PR 검토만 요청된 세션."
fi
echo "</project-context-snapshot>"
